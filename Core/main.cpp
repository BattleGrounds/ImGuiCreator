...
static void SetupVulkanWindow(ImGui_ImplVulkanH_Window* wd, VkSurfaceKHR surface, int width, int height)
{
    ...
    if (res != VK_TRUE)
    {
        ...
        std::cerr << "Error no WST support on physical device 0\n";
        exit(-1);
    }
...
void SimpleButton_Callback(void* _this) {
    UIKitButton* btn = reinterpret_cast<UIKitButton*>(_this);
    cout << btn->title << endl;
}

void SimpleButton2_Callback(void* _this) {
    UIKitButton* btn = reinterpret_cast<UIKitButton*>(_this);
    cout << btn->title << endl;
}

void SlideInt_Callback(void* _this) {
    UIKitSlider2<int>* slider = reinterpret_cast<UIKitSlider2<int>*>(_this);
    cout << slider->getValue()[0] << " " << slider->getValue()[1] << endl;
}

void ImageBtn_Callback(void* _this) {
    UIKitImageButton* btn = reinterpret_cast<UIKitImageButton*>(_this);
    cout << "Settings" << endl;
}
...
int main(int, char**)
{
    ...
    InitImageLoader(init_info, g_MainWindowData);
    // Our state
    bool show_demo_window = true;
    bool show_another_window = false;
    ImVec4 clear_color = ImVec4(0.45f, 0.55f, 0.60f, 1.00f);
    int v = 0;
    UICreatorFormProperties creator_props;
    // Main loop
    UICreatorMain creator;
    bool done = false;
    while (!done)
    {
        ...
        if (ImGui::GetIO().KeyCtrl && ImGui::IsKeyPressed(ImGuiKey_E, false))
            UIKitSetEditMode(!UIKitGetEditMode());
        creator.draw();
